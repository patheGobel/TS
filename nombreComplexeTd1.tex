...
\end{enumerate}
    ...
    \end{enumerate}
\end{enumerate}
\section*{\textcolor{black}{Correction Exercice 3 :}}
\begin{enumerate}
    \item Pour tout complexe $z \neq i$, on pose
    \(
    U = \frac{z + i}{z - i}.
...
\section*{1. Expression de la partie imaginaire de \( U \)}
...
où :
\begin{itemize}
    \item \( A = x^2 + y^2 - 1 \) est la partie réelle du numérateur ;
    \item \( B = 2x \) est la partie imaginaire du numérateur ;
    \item \( D = x^2 + (y-1)^2 \) est le dénominateur (strictement positif sauf en \( (0,1) \), où il s’annule).
\end{itemize}

La partie imaginaire de \( U \) est donc :

\[
\operatorname{Im}(U) = \frac{B}{D} = \frac{2x}{x^2 + (y-1)^2}.
\]

\section*{2. Condition pour que \( \operatorname{Im}(U) < 0 \)}
