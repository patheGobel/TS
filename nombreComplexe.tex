...
\underline{\exemple}\\
Considérons les nombres complexes $z=3+4i$ et $w=-2-7i$. La partie réelle de $z$ est $\mathcal{R}e(z) = 3$, la partie imaginaire est $\mathcal{I}m(z) = 4$. De même, pour $w$, $\mathcal{R}e(w) = -2$ et $\mathcal{I}m(w) = -7$. Les deux nombres complexes sont différents car $\mathcal{R}e(z) \neq \mathcal{R}e(w)$ ou $\mathcal{I}m(z) \neq \mathcal{I}m(w)$.

\underline{\textbf{\textcolor{red}{Remarque }}}\\
Les identités remarquables de $\mathbb{R}$ restent aussi valables dans $\mathbb{C}$, où $a$ et $b$ sont des nombres réels. Cela signifie que pour tout nombre complexe $z=a+ib$, les équations suivantes sont vraies :
\[
  \begin{aligned}
    (a+ib)^{2} &= a^{2}+2iab-b^{2}, \\
    (a-ib)^{2} &= a^{2}-2iab-b^{2}.
...
De même, pour $z$, $z^* = 2-3i$, appliquons l'identité remarquable correspondante :
\[
  \begin{aligned}
    (2-3i)^{2} &= (2)^{2} - 2 \cdot 2 \cdot 3i + (3i)^{2} \\
               &= 4 - 12i - 9 \\
...
Ainsi, l'exemple confirme l'application des identités remarquables dans le contexte des nombres complexes.\\
...
Le conjugué d'un nombre complexe $z = a + ib$ est défini comme $\overline{z} = a - ib$.

\underline{\exemple}

Considérons le nombre complexe $w = 3 + 2i$. Le conjugué de $w$, noté $\overline{w}$, est obtenu en changeant le signe de la partie imaginaire : $\overline{w}=3-2i$.

\underline{\textbf{\textcolor{red}{2 Propriétés}}}\

Le conjugué d'un nombre complexe possède plusieurs propriétés intéressantes :
\begin{itemize}
...
Ces propriétés sont utiles dans divers calculs impliquant des nombres complexes.\\
...
\begin{itemize}
\item[$\bullet$] Le conjugué du conjugué : $\overline{\overline{z_1}} = \overline{2 - 3i} = 2 + 3i = z_1$.
...
Ces exemples illustrent comment les propriétés du conjugué sont vérifiées dans différentes opérations avec des nombres complexes.\\


\section*{\underline{\textbf{\textcolor{red}{III. Représentation graphique d'un Nombre Complexe }}}}

La représentation graphique d'un nombre complexe se fait dans le plan complexe, où l'axe des abscisses correspond à la partie réelle et l'axe des ordonnées à la partie imaginaire.

\subsection*{3.1 Plan Complexes}

Le plan complexe est un plan muni d'un repère cartésien. Chaque nombre complexe $z = a + ib$ peut être associé à un point $(a, b)$ dans ce plan.

Le nombre complexe z = a + i b est appelé l'affixe du point M. On peut donc noter sans ambiguïté M(z) le point M d'affixe z.\\
z est l'affixe de M\\
M est l'image de z\\
On note $M(z)$\\
...
\subsection*{3.2 Représentation d'un Nombre Complexe}
...
Considérons le nombre complexe $z = 3 + 4i$. Le point associé dans le plan complexe est $(3, 4)$.\\
Représente $z$ et $\overline{z}$\\
Cette représentation graphique est utile pour visualiser les opérations sur les nombres complexes et interpréter géométriquement leurs propriétés.

\section*{\underline{\textbf{\textcolor{red}{IV. Module d'un Nombre Complexe}}}}

Le module d'un nombre complexe est une mesure de sa distance à l'origine dans le plan complexe. Soit $z = a + ib$ un nombre complexe, où $a$ et $b$ sont des nombres réels. Le module de $z$, noté $|z|$, est défini comme la distance entre le point représentant $z$ et l'origine $(0,0)$ dans le plan complexe.

\subsection*{\underline{\textbf{\textcolor{red}{4.1 Définition  }}}}


Le module d'un nombre complexe $z = a + ib$ est défini comme :
\[ |z| = \sqrt{a^2 + b^2} \]

\subsection*{\underline{\textbf{\textcolor{red}{4.2 Propriétés}}}}
...
\item[$\bullet$] Le module d'un conjugué : $z\overline{z} = |z|=\sqrt{a^2 + b^2}$.
\item[$\bullet$] Inégalité triangulaire : $|\frac{z_1}{z_2}|= \frac{|z_1|}{|z_2|}$.
\item[$\bullet$] $|z_A - z_B| =AB$.
\end{itemize}
...
\section*{\underline{\textbf{\textcolor{red}{V. Argument et forme trigonométrique d'un nombre complexe non nul }}}}
\subsection*{\underline{\textbf{\textcolor{red}{5.1 Définition}}}}
...
\subsection*{\underline{\textbf{\textcolor{red}{5.2 Propiétés}}}}
...
\subsubsection*{\textbf{1er cas :} $a > 0$}

Si la partie réelle $a$ est positive, l'argument $\theta$ est simplement l'arc tangente de la partie imaginaire $b$ divisée par la partie réelle $a$. Mathématiquement, cela peut être exprimé comme :

\[ \theta = \arctan\left(\frac{b}{a}\right) \]

\subsubsection*{\textbf{2ème cas :} $a < 0$}

Si la partie réelle $a$ est négative, l'argument $\theta$ est ajusté en ajoutant ou soustrayant $\pi$ (180 degrés) selon le signe de la partie imaginaire $b$. Mathématiquement, cela peut être formulé comme :

\[ \theta = \arctan\left(\frac{b}{a}\right) + \pi \quad \text{si } b \geq 0 \]
\[ \theta = \arctan\left(\frac{b}{a}\right) - \pi \quad \text{si } b < 0 \]
\subsection*{Exercice d'application}
...
Si $b<0$ alors $arg(z)=\frac{-\pi}{2}$
\end{itemize}
\subsection*{\underline{\textbf{\textcolor{red}{5.1. Interprétation de $|\frac{d-c}{b-a}|$}}}}
...
\subsection*{\underline{\textbf{\textcolor{red}{5.2 Forme trigonométrique d'un nombre Complexe}}}}
...
\underline{\exemple}\\
Donne la forme trigonométrique des nombres complexes suivantes:\\
$z_{1}=-\sqrt{3}+i$ ; $z_{2}=1+i$ ; $\frac{-\sqrt{2}}{1-i}$\\
\underline{\textbf{\textcolor{red}{Solution}}}\\
\subsection*{\underline{\textbf{\textcolor{red}{5.2 Formule de Moivre }}}}
...
\underline{\textbf{\textcolor{red}{Application : Expression de cosnx et sinnx en fonction de cosx ou sinx  }}}\\

\underline{\textbf{\textcolor{red}{Exercice d'application  }}}\\

\underline{\textbf{\textcolor{red}{ }}}\\
\section*{\underline{\textbf{\textcolor{red}{VI. Forme exponentielle d'un nombre complexe non nul}}}}
\subsection*{\underline{\textbf{\textcolor{red}{6.1 Définition }}}}
 Soit z un nombre complexe non nul d'argument $\theta$. On appelle forme exponentielle d'un nombre complexe l'écriture : $z=|z|e^{i\theta}$\\
\subsection*{\underline{\textbf{\textcolor{red}{6.2 Propriétés }}}}
...
\subsection*{\underline{\textbf{\textcolor{red}{6.3 Formule d'Euler }}}}
A partir de la forme exponentielle d'un nombre complexe on peut déduire les formules\\ d'Euler.\\
$cos(x)$=$\frac{e^{ix}+e^{-ix}}{2}$\\
$sin(x)$=$\frac{e^{ix}-e^{-ix}}{2i}$\\
\subsection*{\underline{\textbf{\textcolor{red}{6.4 Linéariser des expressions trigonométriques:}}}}
...
\subsection*{\underline{\textbf{\textcolor{red}{6.5 Déterminer les racines nièmes $(n\geq2)$ de l'unité:}}}}
\underline{\textbf{\textcolor{red}{6.1.1 Définition}}}\\
soit n un entier naturel non nul et $z$ un nombre complexe non nul.\\ L'écriture 
$z^{n}=\mathcal{Z}$\\
\underline{\textbf{\textcolor{red}{6.1.2 Racine nième d'un nombre complexe}}}\\
