...
\]
    ...
    Si une fonction \( f(x) \) a une asymptote oblique en \( x \to +\infty \) (ou \( x \to -\infty \)), cela signifie que la courbe de la fonction se rapproche d'une droite de pente non nulle. Pour déterminer cette asymptote, on résout
    \[
    \lim_{x \to +\infty} \left( f(x) - (mx + b) \right) = 0
    \]
    où \( m \) est la pente de l'asymptote et \( b \) est l'ordonnée à l'origine.
    
\end{enumerate}

Ces interprétations géométriques sont très utiles pour visualiser le comportement des fonctions à l'infini ou autour de points particuliers.

\textbf{\underline{Exemple graphique :}}\\
